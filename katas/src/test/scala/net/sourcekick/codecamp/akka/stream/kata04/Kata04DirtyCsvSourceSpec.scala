...
class Kata04DirtyCsvSourceSpec extends AsyncWordSpec with Matchers with ParallelTestExecution {
  ...
  "Kata04DirtyCsvSource" must {

    "create the correct stages" in {

      implicit val system = ActorSystem("Kata04")
      implicit val materializer = ActorMaterializer()

      val lineSource = Kata04DirtyCsvSource.createSourceOfLinesInFileKata04Csv() // Source[String, Future[IOResult]]

      val tempFlow = Kata04DirtyCsvSource.createRobustFlowOfStringToTemperatureReading() // Flow[String, TemperatureReading, NotUsed]
